...
class Flock:
    ...
    def graze_all(self, data, left, wrap):
        ...
        for idx in np.ndindex(data.shape):
            new_data[idx] = self.chicks[0].graze(idx, data, left, wrap, edge=True)
        return new_data
    ...
    def graze_all(self, data, left, wrap):
        ...
        if self.chicks[0].mode =='slice':
            test = True
        c_range = reduce_mnc(self.chicks[0].raptor.neighbor_count, data.ndim, test)
        ...
        for i in range(mid[0].size):
            ...
            new_data[idx] = self.chicks[0].graze(idx, data, left, wrap, edge=False)
